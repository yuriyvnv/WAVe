...
run_name = f"whisper_small_mixed_cv_pt_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
MODEL_NAME = "yuriyvnv/whisper-small-mixed-pt"
...
forced_decoder_prompt_ids = processor.get_decoder_prompt_ids(language="pt", task="transcribe")
    ...
    with torch.no_grad():
        for sample in tqdm(split_data, desc=f"Processing {split_name}"):
            ...
            generated_ids = model.generate(
                ...
                forced_decoder_ids=forced_decoder_prompt_ids,
                max_new_tokens=444,
                temperature=0.0,
                num_beams=3,
