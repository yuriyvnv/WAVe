...
# Set up logging
run_name = f"whisper_large_multi_eval_nl_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
MODELS = [
    ...
    "yuriyvnv/whisper-large-v3-mixed-cv-nl", 
    "yuriyvnv/whisper-large-v3-cv-only-nl",
...
def load_datasets():
    """Load both FLEURS and multilingual_librispeech datasets for Dutch"""
    print("Loading FLEURS dataset for Dutch (nl_nl)... 🇳🇱")
    try:
        fleurs_dataset = load_dataset("google/fleurs", "nl_nl", trust_remote_code=True)
        fleurs_dataset = fleurs_dataset.cast_column("audio", Audio(sampling_rate=16000))
        
        # Apply preprocessing to FLEURS (uses "transcription" column)
        fleurs_dataset["test"] = fleurs_dataset["test"].map(
            lambda x: prepare_dataset(x, "transcription"),
            desc="apply preprocessing FLEURS test",
        )
        print(f"✅ FLEURS loaded: {len(fleurs_dataset['test'])} test samples")
    except Exception as e:
        print(f"❌ Error loading FLEURS: {e}")
        fleurs_dataset = None

    print("Loading Multilingual LibriSpeech dataset for Dutch... 📚")
    try:
        mls_dataset = load_dataset("facebook/multilingual_librispeech", "dutch", trust_remote_code=True)
    ...
    return fleurs_dataset, mls_dataset
...
def main():
    # Load datasets once
    fleurs_dataset, mls_dataset = load_datasets()
    ...
    for model_idx, model_name in enumerate(MODELS):
        ...
        try:
            ...
            processor = WhisperProcessor.from_pretrained(model_name, language="nl", task="transcribe")
        ...
        # Evaluate on FLEURS if available
        if fleurs_dataset is not None:
            try:
                fleurs_results = evaluate_model_on_dataset(
                    model, processor, 
                    fleurs_dataset["test"], 
                    "FLEURS", 
                    "transcription",
                )
                model_results["fleurs"] = fleurs_results
                print(f"📊 FLEURS WER: {fleurs_results['wer']:.4f} ({fleurs_results['wer']:.2%})")
            except Exception as e:
                print(f"❌ Error evaluating on FLEURS: {e}")
    ...
    filename = f"whisper_multi_model_evaluation_nl_{timestamp}.json"
